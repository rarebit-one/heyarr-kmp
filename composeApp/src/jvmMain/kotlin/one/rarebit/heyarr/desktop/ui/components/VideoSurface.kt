        ...
        drawIntoCanvas { canvas ->
            renderer.withFrame { f ->
                ...
                canvas.nativeCanvas.drawImageRect(
                    f.image,
                    Rect.makeWH(f.width.toFloat(), f.height.toFloat()),
                    Rect.makeXYWH(left, top, dw, dh),
                    SamplingMode.LINEAR,
                    null,
                    true,
                )
...
fun PlaybackHost(session: AppSession) {
    ...
    LaunchedEffect(item.assetId, playback.player.state.eof) {
        if (playback.loadedAssetId == item.assetId && playback.player.state.eof &&
            playback.audioQueue.isNotEmpty()
        ) {
            playback.next()?.let(playback::play)
        }
    }
    LaunchedEffect(item.assetId, playback.pendingStart) {
        ...
        val err = session.io {
            ...
            if (playback.player.isRunning && playback.loadedAssetId == item.assetId) {
                playback.player.switchTo(embedded)
            } else if (playback.player.isRunning) {
                playback.resolvePlaybackTarget(item).let { t ->
                    playback.player.load(t.url, playback.title(item), t.durationSeconds, t.streamBaseUrl)
                    null
                }
            } else {
                playback.resolvePlaybackTarget(item).let { t ->
        ...
        if (err == null) {
            playback.loadedAssetId = item.assetId
            playback.player.play()
            playback.refreshSubtitles()
        }
    }
}
