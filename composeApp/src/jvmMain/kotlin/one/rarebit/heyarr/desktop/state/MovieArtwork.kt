...
object MovieArtwork {
    fun select(key: MetaKey, hits: List<DiscoveryHit>): ExternalMeta? {
        ...
        val hit = hits.filter { it.type == "movie" && it.source == "tmdb" && title(it.title) == title(key.title) && (key.year == null || key.year == it.year) }
        ...
        return ExternalMeta(imageUrl = image(hit.posterUrl), landscapeImageUrl = backdrop, synopsis = hit.overview, source = "TMDB", sourceUrl = hit.externalId?.let { "https://www.themoviedb.org/movie/$it" })
    }
}
