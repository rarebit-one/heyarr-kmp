...
class LandscapeMetadataTest {
    @Test fun refreshesLegacyCacheAndPersistsBothArtworkRoles() = runBlocking {
        ...
        try {
            val oldKey = MessageDigest.getInstance(
                "SHA-256",
            ).digest("SERIES|example|2025||".toByteArray()).joinToString("") {
                "%02x".format(it)
            }.take(32)
            dir.resolve(
                "$oldKey.json",
            ).writeText("""{"image":"https://art/old","source":"TVmaze","fetched_at":${System.currentTimeMillis()}}""")
            val calls = mutableListOf<String>()
            val metadata = DesktopExternalMetadata.create(cacheDir = dir, fetch = { url ->
                calls += url
                if (url.endsWith("/images")) {
                    """[{"type":"background","resolutions":{"original":{"width":1920,"height":1080,"url":"https://art/wide"}}}]"""
                } else {
                    """{"id":123,"image":{"original":"https://art/poster"},"summary":"Example"}"""
                }
            })
            ...
            val cached = DesktopExternalMetadata.create(cacheDir = dir, fetch = {
                error("Fresh disk cache must avoid network")
            }).lookup(key)!!
