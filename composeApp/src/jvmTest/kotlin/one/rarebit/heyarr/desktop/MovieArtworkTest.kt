...
class MovieArtworkTest {
    ...
    @Test fun refusesWrongYearTitleTypeAndAmbiguousRemakes() {
        ...
        assertNull(MovieArtwork.select(MetaKey(MediaType.MOVIE, "Alien Romulus", 2024), hits.map { it.copy(type = "tv_series") }))
        assertNull(MovieArtwork.select(MetaKey(MediaType.MOVIE, "Alien Romulus"), hits + hits[0].copy(externalId = "other", year = 2025)))
    }
}
