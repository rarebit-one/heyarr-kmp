...
class RemoveWorkTest {
    @Test fun removesOnlySelectedWorkWithCredential() {
        ...
        val transport = object : HttpTransport {
            ...
            override fun post(url: String, body: String?, contentType: String?, headers: Map<String,String>) = HttpResponse(200, Fixtures.rpc("""{"sources":[]}"""))
    ...
    @Test fun discoveryFailureDoesNotDeleteAnything() {
        val transport = object : HttpTransport {
            ...
            override fun post(url: String, body: String?, contentType: String?, headers: Map<String,String>) = HttpResponse(503, "")
    ...
    @Test fun unfollowsOnlyThisWorkBeforeRemovingIt() {
        ...
        val transport = object : HttpTransport {
            ...
            override fun post(url: String, body: String?, contentType: String?, headers: Map<String,String>): HttpResponse {
                if (body.orEmpty().contains("list_followed")) return HttpResponse(200, Fixtures.rpc("""{"sources":[{"id":"mine","title":"Mine","work_id":"selected"},{"id":"other","title":"Other","work_id":"untouched"}]}"""))
                assertTrue(body.orEmpty().contains("mine")); assertFalse(body.orEmpty().contains("other"))
                assertTrue(body.orEmpty().contains("keep_archive"))
                actions += "unfollow"
                return HttpResponse(200, Fixtures.rpc("{}"))
            }
            override fun delete(url: String, headers: Map<String,String>): HttpResponse { actions += "remove"; return HttpResponse(204, "") }
        }
        HeyarrApi(transport, "https://example.test", Credential.Bearer("fixture")).removeWork("selected")
