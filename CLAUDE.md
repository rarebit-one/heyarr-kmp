...
|--------|--------|-------------------|
| `:core` | `kotlin.multiplatform` (jvm + android when an SDK is present) | The pure client layer: `net/` (`HttpTransport`, `JsonScan`, `JsonEscapes`, `JsonArrays`), `mcp/` (`McpClient`, `JsonWrite`, `McpModels`), `heyarr/` (REST + telemetry models), `auth/` (`Credential`, `ClientMode`, `GuestGate`), `state/` (library status, search grouping, toasts), `library/` (`Series` episode grouping and `Variants`, over the `EpisodeFile` / `CatalogWork` seams each app's models implement), `theme/MediaType`, `crypto/Blake3`, `vault/`, `feeds/`, `discovery/`. |
