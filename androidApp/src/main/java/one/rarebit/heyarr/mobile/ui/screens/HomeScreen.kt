...
fun HomeScreen(
        ...
        scope.launch {
            session.io { a.works() }.fold(
                ...
                onFailure = {
                    ...
                    state.spotlight =
                        RailState.Failed(it.message ?: "failed")
                },
            )
        }
    ...
    fun actionsFor(w: Work): List<CardAction> = buildList {
        personal.onToggleStar?.let { toggle ->
            add(
                CardAction(
                    if (w.id in
                        personal.starredIds
        ...
        personal.onAddToPlaylist?.let { add ->
            add(CardAction("Add to playlist", Icons.Rounded.PlaylistAdd) { add(w) })
        }
    }

    LazyColumn(
        modifier.fillMaxSize(),
        contentPadding = PaddingValues(vertical = Tokens.s4),
        verticalArrangement = Arrangement.spacedBy(28.dp),
    ) {
        if (session.isGuest) {
            item {
                Box(Modifier.padding(horizontal = Tokens.screenPadding)) { GuestBanner(onSignInToSave) }
            }
        }
        item {
        ...
        if (personal.starred.isNotEmpty()) {
            item {
                Rail(
                    ...
                    subtitle = "Decrypted on this phone — the node never sees it",
                    key = {
                        it.id
                    },
                    trailing = personal.onOpenPlaylists?.let { f -> @Composable { GhostButton("Playlists", f) } },
                ) { w ->
            ...
            item {
                MediaScope(t) {
                    ...
                    Rail(
                        ...
                        skeletonWidth = if (square) Tokens.squareWidth else Tokens.posterWidth,
                        key = {
                            it.workId
                        },
                    ) { hit ->
                        ...
                        MediaCard(
                            ...
                            artwork = cover.url, status = session.index.statusOf(hit.workId),
                            onWant = {
                                onWant(hit.workId, hit.title)
                            }, width = if (square) Tokens.squareWidth else Tokens.posterWidth, showBadge = false,
                        )
                    }
                }
            }
                ...
                MediaScope(MediaType.PODCAST) {
                    Rail("Following", state.followed, subtitle = "Standing subscriptions the node polls", emptyText = "You follow nothing yet — add a feed or TVDB series in Settings.", skeletonAspect = CardAspect.SQUARE, skeletonWidth = Tokens.squareWidth, key = {
                        ...
                        MediaCard(
                            ...
                            type,
                            onOpen = {
                                s.workId?.let { onOpen(detailRoute(it, type, s.title, from = "Home")) }
                            },
                            subtitle = s.feedRef,
                            meta = listOf("${s.itemsArchived ?: 0}/${s.itemsKnown ?: 0} archived", s.health),
                            artwork = cover.url,
...
private fun WorkCard(
    ...
    MediaCard(
        w.title, type, onOpen = { onOpen(detailRoute(w.id, type, w.title, from = from)) },
        subtitle =
        w.artist ?: w.author,
        meta = listOf(w.year?.toString()), artwork = cover.url, status = status, onWant = {
            onWant(w.id, w.title)
        }, actions = actions,
    )
}
    ...
    when (val s = state.spotlight) {
        ...
        is RailState.Loaded -> {
            ...
            Hero(
                ...
                description =
                work.synopsis?.take(220)
                    ?: cover.external?.synopsis?.let {
                        it.take(220) +
                ...
                primary = {
                    ...
                    PrimaryButton(theme.ctaLabel, {
                        onOpen(detailRoute(work.id, type, work.title, from = "Home"))
