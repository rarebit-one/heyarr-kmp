...
fun LibraryScreen(
    ...
    fun load() {
        ...
        state.error = null
        scope.launch {
            session.io { session.api.works() }.fold(onSuccess = { state.works = it }, onFailure = {
                state.error =
                    it.message
            })
            state.loading = false
        ...
        SectionHeader(
            ...
            trailing = {
                Row(
                    ...
                    IconButtonRound(Icons.Rounded.GridView, "Grid view", {
                        state.grid = true
                    }, filled = state.grid, size = 36.dp)
                    IconButtonRound(Icons.Rounded.ViewList, "List view", {
                        state.grid = false
                    }, filled = !state.grid, size = 36.dp)
                }
            },
        ...
        when {
            ...
            state.works == null -> LazyVerticalGrid(
                ...
                items(9) {
                    MediaCardSkeleton(
                        ...
                        aspect = MediaThemes.of(state.type ?: MediaType.BOOK).aspect,
                    )
                }
            }
            ...
            state.grid -> LazyVerticalGrid(
                ...
                items(filtered, key = { it.id }) { w ->
                    ...
                    MediaCard(
                        w.title, type, onOpen = {
                            onOpen(detailRoute(w.id, type, w.title, from = "Library"))
                        },
                        subtitle =
                        w.artist ?: w.author,
                        meta = listOf(
                        ...
                        artwork = cover.url, status = session.index.statusOf(w.id), onWant = {
                            onWant(w.id, w.title)
                        }, width = null, modifier = Modifier.fillMaxWidth(),
                    )
                }
            }
                ...
                items(filtered, key = { it.id }) { w ->
                    ...
                    MediaRow(
                        ...
                        subtitle =
                        w.artist ?: w.author,
                        meta = listOf(w.year?.toString(), w.recency?.take(10)),
                        artwork = cover.url,
...
fun DownloadsScreen(
    ...
    fun load() {
        state.error = null
        scope.launch {
            session.io { session.api.desired() }.fold(onSuccess = { state.desired = it }, onFailure = {
                state.error =
                    it.message
            })
            ...
            if (wanted.isNotEmpty()) {
                session.io { session.api.works() }.onSuccess { works ->
                    state.titles =
    ...
    LazyColumn(
        ...
        item {
            Notice(
                "The node reports each want's acquisition state and phase, not a percentage — a transfer's byte count lives in the download client and heyarr does not relay it yet.",
                tone = Tokens.slate,
            )
        }
        item {
            Section("Acquiring", subtitle = "${inFlight.size} wants not yet satisfied", trailing = {
                ...
                when {
                    ...
                    state.desired == null -> Skeleton(Modifier.fillMaxWidth().height(80.dp))
                        ...
                        columns = listOf(
                            TableColumn("Want", width = 200.dp),
                            TableColumn("Status", width = 100.dp),
                            TableColumn("Phase", width = 100.dp),
                            TableColumn("Client", width = 90.dp),
                            TableColumn("Detail", width = 240.dp),
                            TableColumn("", width = 150.dp, alignEnd = true),
                        ),
            ...
            Section(
                ...
                when (val jobs = state.jobs) {
                    null -> Skeleton(Modifier.fillMaxWidth().height(80.dp))
                        ...
                        columns = listOf(
                            TableColumn("Job", width = 150.dp),
                            TableColumn("State", width = 90.dp),
                            TableColumn("Attempts", width = 70.dp, alignEnd = true),
                            TableColumn("Updated", width = 130.dp),
                            TableColumn("Last error", width = 240.dp),
                        ),
